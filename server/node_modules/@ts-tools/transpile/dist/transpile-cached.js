...
function transpileCached(options) {
    ...
    const mtime = (0, node_fs_1.statSync)(filePath).mtime.getTime();
    const cacheFilePath = (0, node_path_1.join)(cacheDirectoryPath, (0, helpers_1.filePathToCacheFileName)(filePath));
    const cachedOutput = readCacheFileSync(cacheFilePath);
    if (cachedOutput &&
        cachedOutput.mtime === mtime &&
        cachedOutput.tsVersion === tsVersion &&
        (0, helpers_1.areEmitCompatible)(cachedOutput, compilerOptions)) {
        return cachedOutput;
    }
    const transpiledOutput = typescript_1.default.transpileModule(typeof fileContents === 'string' ? fileContents : (0, node_fs_1.readFileSync)(filePath, 'utf8'), options);
    writeCacheFileSync(cacheFilePath, {
        ...
        ...(0, helpers_1.filterAffectsEmit)(compilerOptions),
    });
...
function writeCacheFileSync(cacheFilePath, output) {
    try {
        (0, node_fs_1.writeFileSync)(cacheFilePath, JSON.stringify(output, null, 2));
    }
    catch {
        /**/
    }
...
function readCacheFileSync(cacheFilePath) {
    if ((0, node_fs_1.existsSync)(cacheFilePath)) {
        try {
            const cachedOutput = JSON.parse((0, node_fs_1.readFileSync)(cacheFilePath, 'utf8'));
